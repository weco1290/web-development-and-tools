...
export const MESSAGES_TO_USER = {
  ...
  "required-word": "A message is required. Please provide a valid input.",
...
export function fetchAllUsernames() {
  return fetch("/api/logged-in-users", {
    method: "GET",
  })
    .catch((err) => Promise.reject({ error: "network-error" }))
    .then((response) => {
...
export function updateMessage(word) {
  return fetch("/api/message", {
    method: "POST",
    headers: {
...
export function fetchNewMessages(lastTimestamp) {
  return fetch(`/api/message/${lastTimestamp}`, {
    method: "GET",
  })
    .catch((err) => Promise.reject({ error: "network-error" }))
    .then((response) => {
